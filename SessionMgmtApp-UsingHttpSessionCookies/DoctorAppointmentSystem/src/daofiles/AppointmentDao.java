...
public class AppointmentDao {
	 ...
	 public static int save(AppointmentBean ab){  
	        int status=0;  
	        try{  
	            Connection con=ConnectionProvider.getConnection();  		//	Sequence = appoint    //speciality
	            PreparedStatement ps=con.prepareStatement(  				//Table =appointment
	                         "insert into appointment(apid,name,email,contact,age,day,speciality,description,id) values(appoint.nextval,?,?,?,?,?,?,?,?)");  
